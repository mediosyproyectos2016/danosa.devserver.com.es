<?php
function cleanFilterParameter($filter){
  return str_replace("\'", "'", $filter  );
};

function filtrar_productos_function(){


	$args = array(
		'post_type' => 'product', // we will sort posts by date
		'posts_per_page' => -1,
		'numberposts' => -1
	);

	if( isset( $_POST['referencia_de_seccion'] ) ){
		$args['tax_query']['relation'] = 'AND';
		$args['tax_query'][] = array(
			'taxonomy' => 'family',
			'field' => 'term_id',
			'terms' => $_POST['referencia_de_seccion'][0]
		);
		/*
		$args['meta_query'][] = array(
			'key' => 'referencia_de_seccion',
			'value' => $_POST['referencia_de_seccion']
		);
		*/
	}

    $catfiltros = get_cat_filters($_POST['referencia_de_seccion'][0]);
  ...
  if( $query->have_posts() ) :
    ...
    while( $query->have_posts() ): $query->the_post();
		...
		include(locate_template('loop-products.php'));
      ...
      <script type="text/javascript">
        ...
        <?php
		   if(get_current_user_id() == 1 && 1==2){
				echo '<div class="debug filtrosNuevos product-filter">';
...
add_action('wp_ajax_filtrar_productos', 'filtrar_productos_function');
add_action('wp_ajax_nopriv_filtrar_productos', 'filtrar_productos_function');
